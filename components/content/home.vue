<template>
  <div class="pt-12 items-center gap-6 flex flex-wrap sm:flex-nowrap sm:justify-between">
    <div class="text-center w-full sm:w-auto sm:order-last">
      <img
        src="@/assets/img/headshot.jpg"
        alt="My face"
        class="inline-block rounded-full h-32 w-32" />
    </div>
    <div grow>
      <h1 class="font-bold text-2xl text-gray-800">Beautus Sduduzo Gumede</h1>
      <h2 class="font-semibold text-gray-600 text-xl">
        Software Engineer at
        <nuxt-link
          to="https://codeo.co.za"
          target="_blank"
          >Codeo_</nuxt-link
        >
      </h2>
    </div>
  </div>
</template>
